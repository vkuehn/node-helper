...
function runCommand(cmd, args, opts){
  ...
  //inherit - [process.stdin, process.stdout, process.stderr] or [0,1,2]
	...
	try{
		...
		command.on('error', (error) => { log('runCommand Error: ' + error.message); });
...
//recieve from the script
runScript.prototype.recieve = function(){
	return this.messages;
	messages = [];			//it's only a stack of messages
}
...
runScript.prototype.start = function(scriptName) {
  ...
  fs.stat(scriptName, function(err, stat) {
	    if(err === null) {
			...
			log('runScript forked ' + scriptName + ' with pid ' + that.child.pid );
		} else {
		    log('runScript' + scriptName + ' does not exist');
		}
	});
};
		...
		if(data){
			...
			if(data !== undefined){
				if(data.constructor === Array){
					data.forEach(function(d){log(d);});
				}else {
					log(data.toString());	
				}
			} else {
				log('showData recieved undefined');
			}
		}
	} catch (error) {
