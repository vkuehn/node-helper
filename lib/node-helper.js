...
runScript.prototype.start = function(scriptName) {
  ...
  fs.stat(scriptName, function(err, stat) {
	    if(err === null) {
			...
			that.threads[pid] = child;
			log('runScript forked ' + scriptName + ' with pid ' + pid );
		} else {
			log('runScript error:' + scriptName + ' does not exist');
	    }
	});
  return message;
};
...
//only use it to bring processes to deep sleep !
